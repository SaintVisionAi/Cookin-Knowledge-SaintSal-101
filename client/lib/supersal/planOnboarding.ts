...
export function getSupersal​​ ​Onboarding​Message(userTier: string, isFirstTime: boolean = false): OnboardingMessage {
  ...
  if (isFirstTime) {
    switch (userTier) {
      case 'unlimited':
        return {
          content: "🎉 Welcome to Unlimited! I'm SaintSal™, your AI companion. I can now chat unlimited, remember our conversations, and stick around your dashboard. Want me to show you Companion Mode?",
          actions: [
      ...
      case 'core':
        return {
          content: "🔥 CORE TOOLS UNLOCKED! I'm your GOTTA GUY™ now. CRM access granted, Partner dashboard ready, Chrome extension activated. Want me to set up your first CRM workflow?",
          actions: [
      ...
      case 'pro':
        return {
          content: "⚡ PRO SUITE ACTIVATED! Admin dashboards unlocked, webhook triggers ready, team management enabled. I can now route escalations to Slack/Twilio. Ready to scale?",
          actions: [
      ...
      case 'white_label':
        return {
          content: "👑 EMPIRE MODE! Full white-label activated. 10 GHL subaccounts ready, custom branding unlocked, revenue sharing enabled. Let's build your AI empire!",
          actions: [
      ...
      case 'custom':
        return {
          content: "🚀 CUSTOM ENTERPRISE! Full onboarding initiated. Custom domain setup, enterprise integrations, dedicated support activated. Your concierge AI experience begins now.",
          actions: [
      ...
      default:
        return {
          content: "👋 Hi! I'm SaintSal™. You're on the free plan with 2 trial messages. Want to unlock unlimited conversations and Companion Mode?",
          actions: [
            { label: "Upgrade to Unlimited", action: "upgrade", url: "/pricing" },
...
export function triggerPlanActions(action: string, userTier: string) {
  ...
  switch (action) {
    ...
    case 'chrome_extension':
      ...
      window.open('https://chrome.google.com/webstore/detail/saintvision-ai', '_blank');
      break;
