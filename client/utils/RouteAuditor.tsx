...
export class RouteAuditor {
  ...
  private auditButton(button: HTMLElement, index: number): { connected: boolean; issue?: ButtonIssue } {
    ...
    const dataLoc = button.getAttribute('data-loc') || button.getAttribute('class') || 'unknown';
  ...
  private auditLink(link: HTMLAnchorElement, index: number): { connected: boolean; issue?: ButtonIssue } {
    ...
    const dataLoc = link.getAttribute('data-loc') || link.getAttribute('class') || 'unknown';
...
export function AuditDashboard() {
  const { auditCurrentPage, auditAllPages, generateReport, autoFix, clearResults, results, isAuditing } = useRouteAuditor();
  ...
  return (
    <div className="p-6 bg-gray-900 rounded-lg border border-gray-700">
      <h2 className="text-xl font-bold text-white mb-4">🔍 Route Audit Dashboard</h2>
        ...
        <div className="space-y-4">
          ...
          {results.map((result, index) => (
            <div key={index} className="bg-gray-800 p-4 rounded border border-gray-600">
              <div className="flex items-center gap-2 mb-2">
                <span className={`text-lg ${result.routeStatus === 'healthy' ? 'text-green-500' : result.routeStatus === 'warning' ? 'text-yellow-500' : 'text-red-500'}`}>
                  {result.routeStatus === 'healthy' ? '✅' : result.routeStatus === 'warning' ? '⚠️' : '❌'}
                </span>
                <span className="font-semibold text-white">{result.page.toUpperCase()}</span>
              </div>
              <div className="text-sm text-gray-300">
                <p>Total Elements: {result.totalButtons}</p>
                <p>Connected: {result.connectedButtons} ({((result.connectedButtons / result.totalButtons) * 100).toFixed(1)}%)</p>
