...
export class ToolValidationService {
  ...
  async validateAllTools(): Promise<Record<string, boolean>> {
    ...
    const validationPromises = Object.keys(TOOL_REGISTRY).map(async (toolId) => {
  ...
  async healthCheck(): Promise<{
    ...
    const developmentTools = tools.filter(t => t.status === 'development').length;
    const placeholderTools = tools.filter(t => t.status === 'placeholder').length;
    const validatedTools = Object.values(validationResults).filter(Boolean).length;
...
export const validateTool = (toolId: string) => ToolValidationService.getInstance().validateTool(toolId);
export const validateAllTools = () => ToolValidationService.getInstance().validateAllTools();
export const getSafeNavigationUrl = (toolId: string) => ToolValidationService.getInstance().getSafeNavigationUrl(toolId);
export const getToolByGuid = (guid: string) => ToolValidationService.getInstance().getToolByGuid(guid);
export const toolHealthCheck = () => ToolValidationService.getInstance().healthCheck();

// Tool click handler with validation
export const handleSafeToolClick = async (toolName: string, navigate: (url: string) => void) => {
  // Find tool by name (fallback to GUID lookup)
  const toolEntry = Object.entries(TOOL_REGISTRY).find(([_, tool]) => tool.name === toolName);
