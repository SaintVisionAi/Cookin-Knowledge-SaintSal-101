...
export class PageAuditor {
  ...
  async auditAllPages(): Promise<PageAuditSummary> {
    const pages = Object.entries(ROUTE_MAP).filter(([_, config]) => 
      !('external' in config && config.external)
    );
  ...
  private determinePageHealth(issues: string[]): 'healthy' | 'warning' | 'error' {
    if (issues.length === 0) return 'healthy';
    if (issues.length <= 2 && !issues.some(i => i.includes('not found'))) return 'warning';
    return 'error';
  }
  ...
  private generateRecommendations(results: any[]): string[] {
    ...
    if (errorResults.length > 0) {
      recommendations.push(`🚨 Fix ${errorResults.length} critical routing errors immediately`);
    }
    
    if (warningResults.length > 0) {
      recommendations.push(`⚠️ Address ${warningResults.length} routing warnings before deployment`);
    }
    
    // Check for missing routes
    const missingRoutes = this.findMissingCommonRoutes();
    if (missingRoutes.length > 0) {
      recommendations.push(`➕ Consider adding missing common routes: ${missingRoutes.join(', ')}`);
    }
    
    // Performance recommendations
    if (results.length > 20) {
      recommendations.push('📦 Consider route code-splitting for better performance');
    }
  ...
  private findMissingCommonRoutes(): string[] {
    ...
    const existingRoutes = Object.values(ROUTE_MAP).map(config => config.path);
  ...
  static quickHealthCheck(): { status: string; issues: string[] } {
    ...
    buttons.forEach(button => {
      ...
      if (!element.onclick && !element.hasAttribute('data-route') && !element.closest('form')) {
        brokenButtons++;
      }
    });
